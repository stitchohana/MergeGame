...
async function main() {
	...
	// Geometric matte only: remove the screenshot bleed outside the straight rails.
	for (let y = 18; y < 860; y++) {
		for (let x = 0; x < 8; x++) board[(y * info.width + x) * 4 + 3] = 0;
		for (let x = info.width - 8; x < info.width; x++) board[(y * info.width + x) * 4 + 3] = 0;
	}
	for (let y = 888; y < info.height; y++) {
		for (let x = 18; x < info.width - 18; x++) board[(y * info.width + x) * 4 + 3] = 0;
	}
