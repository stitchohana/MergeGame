...
完成整个周天时统一发放。当前配置共有
666 个周天。凡人三个周天分别发放 1、0、2 个设施，练气期起每个周天固定发放
2 个非灵石矿设施，共 1329 个周天设施奖励；凡人第 2 周天另发放 10 灵石。

为平滑前期节奏，凡人和练气期每个周天固定 4 个穴位。凡人每个穴位消耗 5 点灵气，
三轮周天合计消耗 60 点；首阶段七个固定引导订单按 1:1 产出 65 点灵气，保留 5 点缓冲。
...
同时配置的发射器不视为“已先发放”。转换脚本会自动校验这条规则；初始棋盘不作为
教程设施发放基线，制作台必须等待其依赖的发射器从更早的周天奖励中发放。

| 字段 | 类型 | 说明 |
|------|------|------|
| `stages` | array | 阶段列表 `[{cultivation_level, name, acupoints, qi_cost, circulation_reward}]` |
| `circulation_reward` | object/number | 整个周天完成时发放的奖励，包含周天经验、体力、灵石和设施 `items`；凡人三个周天分别为1、0、2件设施，其余为2件 |
