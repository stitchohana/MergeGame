...
移除穴位设施奖励和全局生产奖励表，将设施奖励统一放入阶段周天奖励；初始教程设施直接由初始棋盘提供；把 `circulation_exp` 改为 `circulation_reward`，并将全部经验统一到周天完成时发放。周天扩展为 666 个，每个周天发放两个按境界递增等级的设施，突破只发放 1 级灵石矿。
...
2. 将凡人阶段原有的三个教程设施加入 `initial_setup.xlsx`/`initial_setup.json`，激活穴位只发放体力，穴位经验并入周天完成奖励。
...
- home meridian workbook header 为 `cultivation_level, name, acupoints, qi_cost, circulation_reward`。
...
- 凡人和练气期每个周天均为 4 个穴位；前期按层级调整周天消耗与经验，凡人总经验 12，低于练气一层的 48。
