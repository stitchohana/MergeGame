...
function createGMRouter(storage, engine, jwtSecret, gmKey) {
    ...
    router.post("/exec", async (req, res) => {
        try {
            ...
            await (0, queue_1.enqueue)(userId, async () => {
                ...
                switch (cmd) {
                    ...
                    case "levelup": {
                        ...
                        if (c.current_level >= maxLevel) {
                            res.status(400).json({ error: "already_max" });
                            return;
                        }
                        c.current_level += 1;
                    ...
                    case "add_item": {
                        ...
                        if (!itemDef) {
                            res.status(400).json({ error: "item_not_found" });
                            return;
                        }
                        ...
                        const gitem = { uid, id: iid, col: emptyCol, row: emptyRow };
                        if (engine.isLauncher(itemDef)) {
