...
import { GameState, GridItem, PouchItem, CultivationData, QuestType, TokenType, RewardConfig, BattleMonster } from "../storage/interface";
...
interface MeridianThreshold {
 ...
 count_min?: number;
 count_max?: number;
 acupoint_rewards?: RewardConfig | number;
}
...
interface ExpeditionStage {
 name: string;
 monsters: { monster_id: number; count: number }[];
 boss?: { monster_id: number };
}
...
interface HomeMeridianStage {
 name: string;
 acupoints: number;
 qi_cost: number;
 acupoint_rewards?: RewardConfig | number;
 circulation_rewards?: RewardConfig | number;
}
...
export class GameEngine {
 ...
 staminaConfig: StaminaConfig = { max: 100, spawnCost: 10, regenInterval: 120, regenAmount: 1 };
 questResetHour = 0;
 private shopConfig: ShopConfig = { shopItems: [], sellPrices: {}, buyPrices: {} };
 private effectsById = new Map<number, EffectDef>();
 private meridianThresholds: MeridianThreshold[] = [];
 private maps = new Map<number, ExpeditionMap>();
 private monsters = new Map<number, MonsterDef>();
 private rewardsTable = new Map<number, RewardConfig>();
 private homeMeridianDefs: HomeMeridianStage[] = [];
 ...
 switchBoard(state: GameState, boardType: string, battleMapId?: number, battleStage?: number): { ok: true; } | { ok: false; reason: string } {
 if (boardType === this._activeBoard) {
 // Already on the target board — just return console.log(`[engine] board already ${boardType}: ${state.grid.length} items`);
 return { ok: true };
 }
 ...
 console.log(`[engine] board switched to battle: restored ${state.grid.length} battle items, saved ${state.saved_grid.length} main items`);
 } else {
 state.grid = this._buildInitialGrid("battle");
 this._assignUids(state, state.grid);
 console.log(`[engine] board switched to battle: new battle grid ${state.grid.length} items, saved ${state.saved_grid.length} main items`);
 }
 ...
 if (!state.saved_grid?.length) {
 this._assignUids(state, state.grid);
 }
 state.saved_grid = undefined;
 this._activeBoard = "main";
 state.board_type = "main";
 console.log(`[engine] board switched to main: restored ${state.grid.length} main items, saved ${state.battle_grid.length} battle items`);
 } return { ok: true };
 }
 ...
 private _assignUids(state: GameState, grid: GridItem[]): void {
 for (const item of grid) {
 if (!item.uid) {
 item.uid = this._nextUid(state);
 }
 }
 }
 ...
 toCol: number,
 toRow: number
 ): { ok: true; resultUid: number; resultId: number; fromCol: number; fromRow: number; toCol: number; toRow: number } | { ok: false; reason: string } {
 ...
 console.log(`[engine] merge: ${fromName}x2 -> ${mergedName} | grid=${state.grid.length}/63`);
 ...
 launcherCol: number,
 launcherRow: number
 ): { ok: true; spawnedUid: number; spawnedId: number; spawnedName: string; targetCol: number; targetRow: number; charges: number; maxCharges: number; rechargeTime: number }
 | { ok: false; reason: string } {
 const map = this.gridToMap(state.grid);
 ...
 console.log(`[engine] spawn: launcher #${launcherItem.id} -> ${spawnResult.name} at (${target.col},${target.row})`);
...
console.log(`[engine] move: ${itemName} (${fromCol},${fromRow}) -> (${toCol},${toRow})`);
 ...
 const ingName = this.getItemData(removed.id)?.name ?? ("#" + removed.id);
 console.log(`[engine] craft remove: ${ingName} (uid=${removed.uid}) from table -> grid (${targetCol},${targetRow}) | stored=${stored.length}`);
 ...
 state.grid.push({ uid: removed.uid, id: removed.id, col: targetCol, row: targetRow });
 ...
 tableCol: number,
 tableRow: number
 ): { ok: true; recipe: RecipeDef } | { ok: false; reason: string } {
 ...
 console.log(`[engine] craft start: "${validation.recipe.name}" -> ${resultName} | time=${validation.recipe.craft_time}s`);
 ...
 tableCol: number,
 tableRow: number
 ): { ok: true; resultUid: number; resultId: number; } | { ok: false; reason: string } {
 let tableItem = state.grid.find(
 (item) => item.col === tableCol && item.row === tableRow
 );
 ...
 console.log(`[engine] craft retrieve: -> ${retrievedName} uid=${craftUid} at (${target?.col ?? -1},${target?.row ?? -1})`);

 this.questEngine.incrementQuestProgress(state, QuestType.CRAFT, 1, this);

 return { ok: true, resultUid: craftUid, resultId };
 }
 ...
 fromCol: number,
 fromRow: number
 ): { ok: true; matched: boolean; } | { ok: false; reason: string } {
 ...
 console.log(`[engine] breakthrough: -> ${stageName} | qi=${newCultivation.max_qi}`);
 ...
 console.log(`[engine] consume exp pill: ${pillName} | exp +${expGain}`);
 ...
 state: GameState,
 uid: number
 ): { ok: true; pouch: PouchItem[]; col: number; row: number } | { ok: false; reason: string } {
 const idx = state.pouch.findIndex((p) => p.uid === uid);
 if (idx < 0) return { ok: false, reason: "item_not_in_pouch" };
 const map = this.gridToMap(state.grid);
 const target = this.findNearestEmpty(map, 3, 3);
 if (!target) return { ok: false, reason: "no_empty_cell" };
 const item = state.pouch[idx];
 state.pouch.splice(idx, 1);
 state.grid.push({ uid: item.uid, id: item.id, col: target.col, row: target.row });
 console.log(`[engine] pouch withdraw: #${item.id} (uid=${item.uid}) at (${target.col},${target.row}) | pouch=${state.pouch.length} items`);
 return { ok: true, pouch: state.pouch, col: target.col, row: target.row };
 }
 ...
 battleHeal(state: GameState, itemId: number, uid: number, effectId: number): { ok: true; grid: GridItem[] } | { ok: false; reason: string } {
 const idx = state.grid.findIndex(g => g.uid === uid);
 if (idx < 0) return { ok: false, reason: "item_not_found" };
 state.grid.splice(idx, 1); console.log(`[engine] battle heal: #${itemId} removed, uid=${uid}`);
 return { ok: true, grid: state.grid };
 }
 ...
 } console.log(`[engine] battle attack: ${monsterName} killed by #${itemId} | loot=${loot.join(",")} | stage_complete=${stageComplete}`);
 ...
 } console.log(`[engine] battle attack: ${monsterName} took ${dmg} dmg (${state.battle_monsters[mIdx].hp}/${state.battle_monsters[mIdx].max_hp})`);
 ...
 state.last_stamina_tick = Date.now(); console.log(`[engine] consume stamina pill: #${pillId} (uid=${uid}) | stamina +${amount} total=${state.stamina}`);
 ...
 pushAndPlace(state: GameState, fromCol: number, fromRow: number, toCol: number, toRow: number): { ok: true; pushed_col: number; pushed_row: number; from_col: number; from_row: number; to_col: number; to_row: number } | { ok: false; reason: string } {
 if (!this.isInBounds(fromCol, fromRow)) return { ok: false, reason: "source_out_of_bounds" };
 if (!this.isInBounds(toCol, toRow)) return { ok: false, reason: "target_out_of_bounds" };
 const fromKey = this.posKey(fromCol, fromRow);
 const toKey = this.posKey(toCol, toRow);
 console.log(`[engine] pushAndPlace: from=(${fromCol},${fromRow}) to=(${toCol},${toRow}) `);
 ...
 return { ok: true, pushed_col: pushedCol, pushed_row: pushedRow, from_col: fromCol, from_row: fromRow, to_col: toCol, to_row: toRow };
 }

 // --- Meridian ---

 completeMeridianAcupoint(state: GameState, index: number, itemIds: number[]): { ok: true; meridian_acupoints: any[]; qi_gained: number; qi_full: boolean; grid: any[]; cultivation: any; rewards_applied: RewardConfig } | { ok: false; reason: string } {
 ...
 return { ok: true, meridian_acupoints: state.meridian_acupoints, qi_gained: qiGained, qi_full: qiFull, grid: state.grid, cultivation: state.cultivation, rewards_applied: rewardsApplied };
 }
 ...
 depositItem(state: GameState, storageCol: number, storageRow: number, itemId: number, fromCol: number, fromRow: number): { ok: true; } | { ok: false; reason: string } {
 ...
 s.items.push({ uid: sourceItem.uid!, id: itemId }); console.log(`[engine] deposit: #${itemId} (uid=${sourceItem.uid}) -> storage at (${storageCol},${storageRow}) | slots=${s.items.length}/${s.max_slots}`);
 return { ok: true };
 }

 withdrawItem(state: GameState, storageCol: number, storageRow: number, uid: number, targetCol: number, targetRow: number): { ok: true; uid: number; col: number; row: number; } | { ok: false; reason: string } {
 if (!this.isInBounds(storageCol, storageRow)) return { ok: false, reason: "storage_out_of_bounds" };
 ...
 const stored = storageItem.storage.items[idx];
 storageItem.storage.items.splice(idx, 1);

 state.grid.push({ uid: stored.uid, id: stored.id, col: targetCol, row: targetRow });
 const itemName = this.getItemData(stored.id)?.name ?? ("#" + stored.id);
 console.log(`[engine] withdraw: ${itemName} (uid=${stored.uid}) from storage at (${storageCol},${storageRow}) -> (${targetCol},${targetRow})`);
 return { ok: true, uid: stored.uid, col: targetCol, row: targetRow };
 }
 ...
 if (!this.shopConfig.shopItems.some(s => s.id === itemId)) return { ok: false, reason: "not_in_shop" };
